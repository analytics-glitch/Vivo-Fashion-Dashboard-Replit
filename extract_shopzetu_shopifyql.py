...
VENDORS = ["Vivo","vivo","VIVO","Safari","safari","Safari by Vivo","SafariByVivo","Zoya","zoya","Zoya Essentials","Shiv","shiv"]
...
def fetch_shopifyql(since, until):
    all_rows, columns, shopify_totals = [], [], {}
    offset, page = 0, 1
    while True:
        ...
        parsed = [dict(zip(col_names, r)) if isinstance(r, list) else r for r in raw_rows]
        if page == 1:
            for row in parsed:
...
def transform_rows(rows):
    ...
    for r in rows:
        ...
        cust_type   = str(r.get("new_or_returning_customer") or "")
        ...
        result.append((
            ...
            None, cust_type, sale_kind, title, sku, qty_ordered,
            round(price,2), round(price,2),
            round(gross_sales if not is_reversal else 0, 2),
            round(abs(discounts) if not is_reversal else 0, 2),
            round(net_sales, 2),
...
def main():
    ...
    if totals.get("gross_sales"):
        ...
        log.info("Reconciliation: " + ("OK" if pct < 5 else "MISMATCH"))
    if args.dry_run:
