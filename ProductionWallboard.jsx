...
function statusColor(status) {
  switch (status) {
    case "On track":
      return { bg: "#0f7a3d", fg: "#ffffff", label: "ON TRACK" };
    case "Slightly behind":
      return { bg: "#c98a00", fg: "#ffffff", label: "SLIGHTLY BEHIND" };
    case "Behind":
      return { bg: "#c0392b", fg: "#ffffff", label: "BEHIND" };
    case "Not started":
      return { bg: "#555555", fg: "#ffffff", label: "NOT STARTED" };
    default:
      return { bg: "#555555", fg: "#ffffff", label: String(status || "—") };
  }
}
  ...
  return (
    ...
    >
      ...
      >
        <div style={{ display: "flex", alignItems: "baseline", gap: 16 }}>
          <span style={{ fontSize: 46, fontWeight: 800, letterSpacing: 1 }}>
            LINE {data.sewing_line}
          </span>
          <span style={{ fontSize: 20, opacity: 0.8, fontWeight: 500 }}>
            {data.style || ""}
          </span>
        </div>
        <span
      ...
      >
        <Metric
          label="MADE SO FAR"
          value={data.made_so_far}
          sub={`of ${data.daily_target}`}
        />
        <Metric
          label="% OF TARGET"
          value={`${pct}%`}
          sub={`should be ${data.expected_by_now}`}
        />
        <Metric
          label="PACE / HOUR"
          value={data.pace_per_hour}
          sub={`${data.hours_completed}/${data.productive_hours} hrs`}
        />
        <Metric
          label="PROJECTED"
          value={data.projected_landing}
          sub={`${projPct}% of target`}
          highlight={sc.bg}
        />
      </div>
      ...
      <div
        style={{
          display: "grid",
          gridTemplateColumns: `repeat(${data.slots.length || 8}, 1fr)`,
          gap: 8,
        }}
      >
        {data.slots.map((s) => {
          ...
          return (
            ...
            >
              <div style={{ fontSize: 13, opacity: 0.75 }}>{s.slot}</div>
              <div style={{ fontSize: 30, fontWeight: 800, lineHeight: 1.1 }}>
                {a == null ? "—" : a}
              </div>
              <div style={{ fontSize: 12, opacity: 0.7 }}>/{t}</div>
            </div>
          );
        })}
      </div>
...
function Metric({ label, value, sub, highlight }) {
  return (
    ...
    >
      ...
      <div style={{ fontSize: 14, opacity: 0.75 }}>{sub}</div>
    </div>
  );
}
...
export default function ProductionWallboard() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      const { data: first } = await api.get("/production/hourly-tracker", {
        forceFresh: true,
      });
      const day = first.work_date;
      const avail =
        first.available_lines && first.available_lines.length
          ? first.available_lines
          : [first.sewing_line];
      // Fetch each line for that day.
      const results = await Promise.all(
        avail.map((ln) =>
          api
            .get(`/production/hourly-tracker`, {
              params: { work_date: day, sewing_line: ln },
              forceFresh: true,
            })
            .then((r) => r.data),
        ),
  ...
  return (
    ...
    >
      ...
      {err && (
        <div style={{ color: "#ffb4a8", fontSize: 20, padding: 20 }}>{err}</div>
      )}
      ...
      <div
        style={{
          display: "grid",
          gridTemplateColumns:
            lines.length > 1 ? "repeat(auto-fit, minmax(560px, 1fr))" : "1fr",
          gap: 22,
        }}
      >
        {lines.map((d) => (
          <LineBoard key={d.sewing_line} data={d} />
        ))}
      </div>

      {!err && lines.length === 0 && (
        <div style={{ color: "#9fd8b8", fontSize: 22, padding: 30 }}>
          No production data yet for today.
        </div>
      )}
    </div>
  );
