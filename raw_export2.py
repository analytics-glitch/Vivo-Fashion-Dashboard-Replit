...
store="zoyaathleisure.myshopify.com"          # correct raw-material domain
token=os.environ["SHOPIFY_RAW_MATERIAL_TOKEN"] # update this secret first
...
def get_all():
    ...
    while url:
        ...
        if r.status_code!=200:
            print("AUTH/REQUEST ERROR",r.status_code,r.text[:200]); raise SystemExit
...
print("product fields:",scalar)
print("variant fields:",sorted(vkeys))
...
for i,p in enumerate(prods):
    ...
    if mr.status_code==429:
        time.sleep(int(mr.headers.get("Retry-After",5)))
        mr=requests.get(f"{BASE}/products/{p['id']}/metafields.json",headers=headers,timeout=60)
...
print("metafield keys:",len(mf_keys),mf_keys[:20])
...
with open(out,"w",newline="",encoding="utf-8") as fh:
    ...
    for p in prods:
        ...
        mfd=mf_by_prod.get(p['id'],{}); mfrow=[flat(mfd.get(k)) for k in mf_keys]
