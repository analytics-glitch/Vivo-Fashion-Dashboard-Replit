...
def register_workforce_desk_routes(app, A):
    ...
    @app.get("/api/workforce-desk/report/latest")
    async def workforce_desk_report_latest():
        import psycopg2
        conn = psycopg2.connect(os.environ["DATABASE_URL"])
        try:
    ...
    @app.post("/api/workforce-desk/report")
    async def workforce_desk_report_generate(request):
        import psycopg2
        conn = psycopg2.connect(os.environ["DATABASE_URL"])
        try:
            api_key = os.environ.get("ANTHROPIC_API_KEY", "")
            ...
            if not ctx:
                ...
                return JSONResponse(
                    {"error": "No coaching context yet. Open the Workforce Desk overview first."},
                    status_code=400)
