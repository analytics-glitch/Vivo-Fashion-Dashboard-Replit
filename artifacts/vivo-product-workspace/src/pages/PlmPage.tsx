...
import { FISCAL_WEEKS_2026, MERCHANDISING_CATEGORIES, subcategoriesForCategory } from '../lib/productTaxonomy';
...
function NewStyleModal({ styles, onClose, onCreated }: { styles: WorkspaceStyle[]; onClose: () => void; onCreated: (id: number) => void }) {
  ...
  const [form, setForm] = useState({ styleNumber: '', name: '', brand: 'Vivo', category: '', subCategory: '', theme: '', orderType: 'New', tier: '2', launchRoute: '', styleClassification: '', rangeTier: '', designUserId: null as number | null, patternUserId: null as number | null, targetDate: '' });
  const update = (key: keyof typeof form, value: string) => setForm((current) => ({ ...current, [key]: value }));
  const updateCategory = (category: string) => setForm((current) => ({ ...current, category, subCategory: '' }));
  const subcategories = subcategoriesForCategory(form.category);
  const teamMembers = teamDirectory.data ?? [];
  ...
  return <div className="plm-modal-backdrop" onClick={onClose}>
    <div className="plm-modal" onClick={(event) => event.stopPropagation()}>
      <header className="plm-modal-head">
        <div><span className="plm-kicker">PLM / New record</span><h2>Start a style</h2><p>Capture the brief before the first handoff.</p></div>
        <button className="plm-close" type="button" onClick={onClose} aria-label="Close new style modal" data-testid="button-close-new-style"><X size={18} /></button>
      </header>
      <form onSubmit={submit}>
        <div className="plm-form-grid">
          <Field label="Style number" hint="Optional internal reference"><input value={form.styleNumber} onChange={(event) => update('styleNumber', event.target.value)} placeholder="e.g. V26-041" data-testid="input-new-style-number" /></Field>
          <Field label="Style name"><input required value={form.name} onChange={(event) => update('name', event.target.value)} placeholder="e.g. Sanaa wrap shirt" data-testid="input-new-style-name" /></Field>
          <Field label="Brand"><select value={form.brand} onChange={(event) => update('brand', event.target.value)} data-testid="select-new-style-brand"><option>Vivo</option><option>Safari by Vivo</option></select></Field>
          <Field label="Category">
            <select required value={form.category} onChange={(event) => updateCategory(event.target.value)} data-testid="select-new-style-category">
              <option value="">Select category</option>
              {MERCHANDISING_CATEGORIES.map((category) => <option key={category} value={category}>{category}</option>)}
            </select>
          </Field>
          <Field label="Sub-category">
            <select value={form.subCategory} onChange={(event) => update('subCategory', event.target.value)} disabled={!form.category} data-testid="select-new-style-subcategory">
          ...
          <Field label="Theme"><input value={form.theme} onChange={(event) => update('theme', event.target.value)} placeholder="e.g. Quiet utility" data-testid="input-new-style-theme" /></Field>
          <Field label="Launch route"><select value={form.launchRoute} onChange={(event) => update('launchRoute', event.target.value)} data-testid="select-new-style-launch-route"><option value="">Not set</option>{LAUNCH_ROUTES.map((value) => <option key={value}>{value}</option>)}</select></Field>
          <Field label="Style classification"><select value={form.styleClassification} onChange={(event) => update('styleClassification', event.target.value)} data-testid="select-new-style-classification"><option value="">Not set</option>{STYLE_CLASSIFICATIONS.map((value) => <option key={value}>{value}</option>)}</select></Field>
          <Field label="Range tier"><select value={form.rangeTier} onChange={(event) => update('rangeTier', event.target.value)} data-testid="select-new-style-range-tier"><option value="">Not set</option>{RANGE_TIERS.map((value) => <option key={value}>{value}</option>)}</select></Field>
          <div className="plm-field"><span>Designer</span><TeamMemberPicker role="design" value={form.designUserId} members={teamMembers} onChange={(value) => setForm((current) => ({ ...current, designUserId: value }))} /></div>
          <div className="plm-field"><span>Pattern maker</span><TeamMemberPicker role="pattern" value={form.patternUserId} members={teamMembers} onChange={(value) => setForm((current) => ({ ...current, patternUserId: value }))} /></div>
          <Field label="Order type"><select value={form.orderType} onChange={(event) => update('orderType', event.target.value)} data-testid="select-new-style-order-type"><option>New</option><option>Repeat</option></select></Field>
          <Field label="Tier"><select value={form.tier} onChange={(event) => update('tier', event.target.value)} data-testid="select-new-style-tier"><option value="1">Tier 1</option><option value="2">Tier 2</option><option value="3">Tier 3</option><option value="4">Tier 4</option></select></Field>
          <Field label="Target Launch">
            <select required value={form.targetDate} onChange={(event) => update('targetDate', event.target.value)} data-testid="select-new-style-target-launch">
              <option value="">Select target launch week</option>
              {FISCAL_WEEKS_2026.map((week) => <option key={week.value} value={week.value}>{week.label}</option>)}
            </select>
          </Field>
        </div>
        {create.isError && <div className="plm-form-error"><CircleAlert size={15} /> This style could not be created. Check the fields and try again.</div>}
        <footer className="plm-modal-actions">
          <button type="button" className="plm-button quiet" onClick={onClose} data-testid="button-cancel-new-style">Cancel</button>
          <button type="submit" className="plm-button primary" disabled={create.isPending} data-testid="button-create-new-style"><Plus size={15} />{create.isPending ? 'Creating…' : 'Create style'}</button>
        </footer>
      </form>
    </div>
  </div>;
...
function DetailDrawer({ id, onClose }: { id: number; onClose: () => void }) {
  ...
  const [techForm, setTechForm] = useState({ basePatternReference: '', fabricId: '', trimsAccessories: '', constructionNotes: '', audacesFileReference: '', modifiedFromStyleNumber: '', status: 'Draft', version: '1.0', ownerUserId: '' });
  const [fitForm, setFitForm] = useState({ sessionDate: '', sample: 'First sample', modelName: '', attendees: '', outcome: 'Needs Revision', comments: '' });
  const [gradingForm, setGradingForm] = useState({ sizeRange: '', cadTeamMemberUserId: '', status: 'Pending' });
  const [sampleForm, setSampleForm] = useState({ purpose: 'Development', patternMakerUserId: '', sampleMakerUserIds: [] as string[], unitsOrdered: '1', dateCut: '', dateFinished: '', status: 'Planned', reworkNotes: '' });
  ...
  const formInput = (value: string, setter: (value: string) => void, props?: Record<string, string>) => <input value={value} onChange={(event) => setter(event.target.value)} {...props} />;
   const detailContent = () => {
    ...
    if (tab === 'Fit Session') return <div className="plm-detail-form"><div className="plm-form-heading"><div><span className="plm-kicker">Fit room</span><h3>Log a fit session.</h3></div><Ruler size={19} /></div><form onSubmit={(event) => { event.preventDefault(); save(fitSession, { ...fitForm }, 'Fit session added'); }}><div className="plm-form-grid"><Field label="Session date"><input required type="date" value={fitForm.sessionDate} onChange={(event) => patch(setFitForm, 'sessionDate', event.target.value)} /></Field><Field label="Sample"><input required value={fitForm.sample} onChange={(event) => patch(setFitForm, 'sample', event.target.value)} /></Field><Field label="Model name"><input required value={fitForm.modelName} onChange={(event) => patch(setFitForm, 'modelName', event.target.value)} /></Field><Field label="Attendees"><input value={fitForm.attendees} onChange={(event) => patch(setFitForm, 'attendees', event.target.value)} placeholder="Names separated by commas" /></Field><Field label="Outcome"><select value={fitForm.outcome} onChange={(event) => patch(setFitForm, 'outcome', event.target.value)}><option>Needs Revision</option><option>Approved</option></select></Field><Field label="Comments"><textarea value={fitForm.comments} onChange={(event) => patch(setFitForm, 'comments', event.target.value)} placeholder="Record the decision and next action…" /></Field></div>{fitSession.isError && <FormError />}{notice && <FormNotice message={notice} />}{formActions(fitSession.isPending, 'Add fit session')}</form><RecordList title="Previous fit sessions" items={(style.fitSessions || []) as unknown[]} /></div>;
     if (tab === 'Grading') return <div className="plm-detail-form"><div className="plm-form-heading"><div><span className="plm-kicker">Size architecture</span><h3>Set grading ownership.</h3></div><Shirt size={19} /></div><form onSubmit={(event) => { event.preventDefault(); save(grading, { ...gradingForm, cadTeamMemberUserId: gradingForm.cadTeamMemberUserId ? Number(gradingForm.cadTeamMemberUserId) : null }, 'Grading updated'); }}><div className="plm-form-grid"><Field label="Size range"><input required value={gradingForm.sizeRange} onChange={(event) => patch(setGradingForm, 'sizeRange', event.target.value)} placeholder="e.g. 6–18" /></Field><Field label="CAD team member"><select value={gradingForm.cadTeamMemberUserId} onChange={(event) => patch(setGradingForm, 'cadTeamMemberUserId', event.target.value)}><option value="">Unassigned</option>{(teamDirectory.data || []).map((member) => <option key={member.id} value={member.id}>{member.name}</option>)}</select></Field><Field label="Status"><select value={gradingForm.status} onChange={(event) => patch(setGradingForm, 'status', event.target.value)}><option>Pending</option><option>In Progress</option><option>Complete</option></select></Field></div>{grading.isError && <FormError />}{notice && <FormNotice message={notice} />}{formActions(grading.isPending, 'Save grading')}</form><RecordList title="Grading records" items={(style.gradings || []) as unknown[]} /></div>;
     if (tab === 'Samples') return <div className="plm-detail-form"><div className="plm-form-heading"><div><span className="plm-kicker">Sample room</span><h3>Register the next sample.</h3></div><Tag size={19} /></div><form onSubmit={(event) => { event.preventDefault(); save(sample, { ...sampleForm, patternMakerUserId: sampleForm.patternMakerUserId ? Number(sampleForm.patternMakerUserId) : null, sampleMakerUserIds: sampleForm.sampleMakerUserIds.map(Number), unitsOrdered: Number(sampleForm.unitsOrdered) }, 'Sample added'); }}><div className="plm-form-grid"><Field label="Purpose"><input required value={sampleForm.purpose} onChange={(event) => patch(setSampleForm, 'purpose', event.target.value)} /></Field><Field label="Pattern maker"><select required value={sampleForm.patternMakerUserId} onChange={(event) => patch(setSampleForm, 'patternMakerUserId', event.target.value)}><option value="">Select Settings user</option>{(teamDirectory.data || []).map((member) => <option key={member.id} value={member.id}>{member.name}</option>)}</select></Field><Field label="Sample makers"><select multiple value={sampleForm.sampleMakerUserIds} onChange={(event) => setSampleForm((current) => ({ ...current, sampleMakerUserIds: Array.from(event.currentTarget.selectedOptions, (option) => option.value) }))}>{(teamDirectory.data || []).map((member) => <option key={member.id} value={member.id}>{member.name}</option>)}</select></Field><Field label="Units ordered"><input required type="number" min="1" value={sampleForm.unitsOrdered} onChange={(event) => patch(setSampleForm, 'unitsOrdered', event.target.value)} /></Field><Field label="Date cut"><input type="date" value={sampleForm.dateCut} onChange={(event) => patch(setSampleForm, 'dateCut', event.target.value)} /></Field><Field label="Date finished"><input type="date" value={sampleForm.dateFinished} onChange={(event) => patch(setSampleForm, 'dateFinished', event.target.value)} /></Field><Field label="Status"><select value={sampleForm.status} onChange={(event) => patch(setSampleForm, 'status', event.target.value)}><option>Planned</option><option>In Progress</option><option>Complete</option><option>Rework</option></select></Field><Field label="Rework notes"><textarea value={sampleForm.reworkNotes} onChange={(event) => patch(setSampleForm, 'reworkNotes', event.target.value)} /></Field></div>{sample.isError && <FormError />}{notice && <FormNotice message={notice} />}{formActions(sample.isPending, 'Add sample')}</form><RecordList title="Sample development" items={(style.samples || []) as unknown[]} /></div>;
...
function AddPlmStyleToAssortment({ style, onClose }: { style: WorkspaceStyle; onClose: () => void }) {
  ...
  const add = async () => {
    ...
    try {
      const response = await fetch('/api/workspace/assortment-plan/add-style', {
        ...
        credentials: 'include',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ season, source: 'pd_styles', pdId: style.id, styleNumber: style.code }),
      });
      const body = await response.json().catch(() => ({}));
  ...
  return <section className="plm-assortment-panel" aria-label="Add to Assortment Plan">
    <div className="plm-section-title"><div><span className="plm-kicker">Planning action</span><h4>Add to Assortment Plan</h4><small>{text(style.code, `ST-${style.id}`)} · {style.name}</small></div><button className="plm-close" type="button" onClick={onClose} aria-label="Close Assortment Plan panel"><X size={15} /></button></div>
    <label className="plm-season-field"><span>Planning quarter</span><select value={season} onChange={(event) => setSeason(event.target.value as typeof season)}><option>Q3 2026</option><option>Q4 2026</option></select></label>
    <button type="button" className="plm-button primary" onClick={add} disabled={pending}><ArrowRight size={14} /> {pending ? 'Adding…' : 'Add style'}</button>
    {notice && <FormNotice message={notice} />}{error && <div className="plm-form-error"><CircleAlert size={15} /> {error}</div>}
  </section>;
}
...
export default function PlmPage() {
  ...
  const [filters, setFilters] = useState<{ tier: string[]; stage: string[]; category: string[]; subCategory: string[]; fabricCategory: string[]; brand: string[]; primaryColour: string[]; edit: string[] }>({ tier: [], stage: [], category: [], subCategory: [], fabricCategory: [], brand: [], primaryColour: [], edit: [] });
  ...
  const choices = useMemo(() => ({
    tier: Array.from(new Set(styles.map((style) => text(style.tier, '—')))).filter((value) => value !== '—'),
    stage: ALL_STAGES,
    category: Array.from(new Set(styles.map((style) => text(style.category, 'Uncategorised')))),
    subCategory: Array.from(new Set(styles.map((style) => text(style.subCategory || style.category, 'Uncategorised')))),
    fabricCategory: Array.from(new Set(styles.map((style) => text((record(style).fabricCategory || record(style).fabricType), 'Unassigned')))),
    brand: Array.from(new Set(styles.map((style) => text(style.brand, 'Unknown')))),
    primaryColour: Array.from(new Set(styles.map((style) => text(record(style).primaryColour || record(style).sampleColour, 'Unassigned')))),
    edit: Array.from(new Set(styles.map((style) => text(record(style).edit || record(style).theme, 'Unassigned')))),
  }), [styles]);
  const filtered = useMemo(() => styles.filter((style) => {
    ...
    return (!query || `${style.code} ${style.name} ${style.category}`.toLowerCase().includes(query))
       && (filters.tier.length === 0 || filters.tier.includes(text(style.tier, '—')))
      ...
      && (filters.category.length === 0 || filters.category.includes(style.category))
       && (filters.subCategory.length === 0 || filters.subCategory.includes(text(style.subCategory || style.category, 'Uncategorised')))
       && (filters.fabricCategory.length === 0 || filters.fabricCategory.includes(text(record(style).fabricCategory || record(style).fabricType, 'Unassigned')))
       && (filters.brand.length === 0 || filters.brand.includes(text(style.brand, 'Unknown')))
       && (filters.primaryColour.length === 0 || filters.primaryColour.includes(text(record(style).primaryColour || record(style).sampleColour, 'Unassigned')))
       && (filters.edit.length === 0 || filters.edit.includes(text(record(style).edit || record(style).theme, 'Unassigned')))
  ...
  return <section className="page plm-page" onClick={() => { setMenuOpen(null); setGroupMenuOpen(false); }}>
    ...
    <div className="plm-toolbar">
      ...
      <div className="plm-filter-label"><Filter size={14} /> Filter by</div>
         {(['tier', 'stage', 'category', 'subCategory', 'fabricCategory', 'brand', 'primaryColour', 'edit'] as const).map((key) => <MultiSelectFilter key={key} variant="plm" label={key === 'subCategory' ? 'Sub-category' : key === 'fabricCategory' ? 'Fabric Category' : key === 'primaryColour' ? 'Primary Colour' : key[0].toUpperCase() + key.slice(1)} options={choices[key]} values={filters[key]} onChange={(next) => setFilters((current) => ({ ...current, [key]: next }))} testId={`select-plm-filter-${key}`} alwaysShowCount />)}
