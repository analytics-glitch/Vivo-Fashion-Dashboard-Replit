...
type Payload = { assortmentStyles: Style[]; assortmentSummary: Summary; quarterSummaries: Record<Quarter, Summary>; quarterStyles?: Record<Quarter, Style[]>; seasons?: Season[]; weeklyDestinations?: Week[]; assortmentFilterOptions?: Record<FilterKey, string[]>; sourceStatus?: unknown; reconciliations?: Reconciliation[] };
...
function StyleCard({ style, selected, onSelect, action }: { style: Style; selected: boolean; onSelect: () => void; action: React.ReactNode }) {
  ...
  return <article className={`assortment-style-card consolidated ${selected ? 'selected' : ''}`}>
    ...
    <div className="assortment-style-copy">
      <div className="assortment-style-topline"><span className="assortment-tier-badge">{style.tier ?? 'Unclassified'}</span><span className="assortment-bi-badge">BI source</span><span className="assortment-status">{style.status}</span></div>
...
export default function AssortmentPlanPage() {
  ...
  const selectedStyles = styles.filter((s) => isBiStyle(s) && selected.includes(s.id)); const counts = filteredAssortmentQuarterCounts(byQuarter, filters); const activeCount = Object.values(filters).flat().length;
