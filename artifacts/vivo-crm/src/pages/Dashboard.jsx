...
import { ArrowRight, CheckCircle2, Search, Calendar, AlertTriangle, Sparkles, Cake, Award, TrendingUp, Gift } from "lucide-react";
...
function KPI({ label, value, sub, delta, deltaInverted, testid }) {
  ...
  return (
    <div className="vivo-card p-6" data-testid={testid}>
      ...
      <div className="font-metric text-5xl mt-4 font-mono-num leading-none">{value}</div>
      {sub && <div className="text-sm text-[var(--vivo-muted)] mt-3">{sub}</div>}
    </div>
  );
}
...
export default function Dashboard() {
  ...
  return (
    <div className="p-6 md:p-10 max-w-[1400px] mx-auto" data-testid="dashboard-page">
      ...
      {user?.role === "manager" && <LoyaltyPulseCard />}
              ...
              <Card key={key} className="vivo-card p-6 rounded-xl" data-testid={`call-list-${key}`}>
                ...
                {loading ? (
                  ...
                  <div className="vivo-empty">
                    <Icon className="h-6 w-6 mx-auto" style={{ color: accent, opacity: 0.5 }} />
                    <h4>All clear here</h4>
                    <p>{emptyMsg}</p>
                  </div>
                ) : (
                  <ul className="divide-y divide-[var(--vivo-border)]">
                    {items.slice(0, 5).map((c) => (
                      <li key={c.customer_id}>
                        <Link to={`/customers/${c.customer_id}`} className="flex items-center justify-between gap-3 min-h-[72px] py-3 group press-effect" data-testid={`call-list-item-${c.customer_id}`}>
                            ...
                            <div className="font-medium truncate group-hover:text-[var(--vivo-navy)] flex items-center gap-2 flex-wrap">
                              ...
                              {c.nba_urgency && (
                                <span className={`text-[10px] uppercase tracking-wider px-2 py-0.5 rounded-full font-semibold ${
                                  c.nba_urgency === "high" ? "bg-[var(--vivo-gold)] text-white" :
                                  c.nba_urgency === "medium" ? "bg-amber-100 text-amber-800" :
                                  "bg-[var(--vivo-bg-soft)] text-[var(--vivo-navy)] border border-[var(--vivo-border)]"
...
function LoyaltyPulseCard() {
  const [pulse, setPulse] = useState(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    let mounted = true;
    api.get("/loyalty/pulse")
  ...
  const stats = [
    {
      key: "upgrade",
      label: "Close to upgrade",
      count: pulse.close_to_upgrade.count,
      hint: `within ${fmtKES(pulse.close_to_upgrade.within_kes)}`,
      tone: "gold",
      icon: TrendingUp,
      top: pulse.close_to_upgrade.top,
      empty: "No customers within striking distance.",
      to: "/loyalty",
    },
    {
      key: "risk",
      label: "Demotion risk · 30d",
      count: pulse.demotion_risk.count,
      hint: "anniversary within 30 days, below retention floor",
      tone: pulse.demotion_risk.count > 0 ? "red" : "muted",
      icon: AlertTriangle,
      top: pulse.demotion_risk.top,
      empty: "No Silver / Gold members at risk.",
      to: "/loyalty",
    },
    {
      key: "vouchers",
      label: "Vouchers expiring · 7d",
      count: pulse.vouchers_expiring.count,
      hint: `${pulse.voucher_activity_30d.redeemed}/${pulse.voucher_activity_30d.issued} redeemed last 30d (${pulse.voucher_activity_30d.redemption_rate}%)`,
      tone: pulse.vouchers_expiring.count > 0 ? "navy" : "muted",
      icon: Gift,
      top: pulse.vouchers_expiring.top,
      empty: "No vouchers expiring this week.",
      to: "/loyalty",
    },
  ];

  const accentByTone = {
    gold: { border: "border-l-[var(--vivo-gold)]", chip: "text-[var(--vivo-navy)] bg-[var(--vivo-bg)]" },
    red:  { border: "border-l-red-500", chip: "text-red-700 bg-red-50" },
    navy: { border: "border-l-[var(--vivo-navy)]", chip: "text-[var(--vivo-navy)] bg-[var(--vivo-bg)]" },
    muted:{ border: "border-l-[var(--vivo-border)]", chip: "text-[var(--vivo-muted)] bg-[var(--vivo-bg)]" },
  };

  return (
    <Card className="vivo-card mt-6 p-6 rounded-xl" data-testid="loyalty-pulse-card">
      <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
        <div className="flex items-center gap-2">
          <Award className="h-4 w-4 text-[var(--vivo-gold)]" />
          <div className="eyebrow">Loyalty pulse</div>
        </div>
        <Link to="/loyalty" className="text-xs text-[var(--vivo-navy)] hover:underline inline-flex items-center gap-1" data-testid="loyalty-pulse-view-all">
          Open Loyalty <ArrowRight className="h-3 w-3"/>
        </Link>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {stats.map((s) => {
          const A = accentByTone[s.tone];
          return (
            <Link
              key={s.key}
              to={s.to}
              className={`block rounded-lg border bg-white p-4 border-l-4 ${A.border} hover:shadow-sm transition`}
              data-testid={`loyalty-pulse-${s.key}`}
            >
              <div className="flex items-start justify-between gap-2">
                <div className="text-[10px] uppercase tracking-[0.18em] text-[var(--vivo-muted)] font-semibold">{s.label}</div>
                <s.icon className="h-3.5 w-3.5 text-[var(--vivo-muted)]"/>
              </div>
              <div className="font-metric text-4xl mt-2 leading-none" data-testid={`loyalty-pulse-${s.key}-count`}>{s.count}</div>
              <div className="text-[11px] text-[var(--vivo-muted)] mt-1">{s.hint}</div>
              {s.top?.length > 0 ? (
                <ul className="mt-3 space-y-1 text-xs">
                  {s.top.map((t, i) => (
                    <li key={i} className="flex items-center justify-between gap-2 border-t border-[var(--vivo-border)] pt-1.5">
                      <span className="truncate text-[var(--vivo-navy)]">{t.customer_name || t.customer_id}</span>
                      <span className={`text-[10px] px-1.5 py-0.5 rounded-sm ${A.chip}`}>
                        {s.key === "upgrade" && <>{fmtKES(t.needed_kes)} to <span className="uppercase">{t.next_tier}</span></>}
                        {s.key === "risk" && <>{t.days_to_anniversary}d · short {fmtKES(t.shortfall_kes)}</>}
                        {s.key === "vouchers" && <>{fmtKES(t.amount_kes)} · {t.expires_at?.slice(0,10)}</>}
                      </span>
                    </li>
                  ))}
                </ul>
              ) : (
                <div className="mt-3 text-[11px] text-[var(--vivo-muted)] italic">{s.empty}</div>
              )}
            </Link>
          );
        })}
