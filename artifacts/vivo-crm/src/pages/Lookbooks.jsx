          ...
          {items.map((lb) => {
            ...
            return (
              <Link key={lb.lookbook_id} to={`/share/${lb.share_token}`} target="_blank" className="vivo-card overflow-hidden hover:shadow-md transition-shadow">
                <div className="w-full aspect-[4/3] bg-[var(--vivo-bg)] overflow-hidden relative">
                  {cover && <ProductTilePlaceholder item={cover} className="w-full h-full" />}
                  {cover?.image && (
                    <img
                      src={cover.image}
                      alt=""
