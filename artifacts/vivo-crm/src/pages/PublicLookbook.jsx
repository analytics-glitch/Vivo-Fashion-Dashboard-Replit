...
export default function PublicLookbook() {
  ...
  // Preserve the associate's curated order when present (added in iter 20).
  const orderedItems = (lb.items || []).slice().sort((a, b) => {
    const ai = typeof a.order === "number" ? a.order : 999;
    const bi = typeof b.order === "number" ? b.order : 999;
    return ai - bi;
  });
          ...
          {orderedItems.map((it) => {
            ...
            return (
              <div key={it.sku} className="bg-white border border-[var(--vivo-border)] overflow-hidden">
                <div className="aspect-[3/4] bg-[var(--vivo-bg)] relative">
                  <ProductTilePlaceholder item={it} className="w-full h-full" />
                  {it.image && (
                    <img
                      src={it.image}
                      alt=""
