...
import { Clock, Star, Gift, ShoppingBag, Receipt, Ticket, Sparkle, MagicWand, VideoCamera, Ruler, UsersThree, CalendarCheck } from "@phosphor-icons/react";
...
export default function TabRewards() {
  ...
  const transactions = [
    ...
    { id: 2, action: "Joined #MyVivoStory Challenge", date: "Aug 08, 2026", pts: "+50", status: "Pending ⏳" },
    { id: 3, action: "Left a photo review", date: "Aug 02, 2026", pts: "+25", status: "Posted" },
    { id: 4, action: "Redeemed 10% Off", date: "Jul 26, 2026", pts: "-500", status: "Posted" },
  ];
    ...
    <div className="animate-in fade-in duration-500 max-w-4xl mx-auto space-y-10">
      ...
      <div className="bg-gradient-to-br from-[#d4af37] via-[#c25e30] to-[#8a3818] rounded-3xl p-8 sm:p-12 text-center text-white shadow-xl relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <div className="text-6xl sm:text-7xl font-black mb-4 tracking-tight">{points.toLocaleString()} <span className="text-2xl font-bold opacity-80">pts</span></div>
          <TierBadge tier={currentUser.tier} className="text-sm px-4 py-1.5 shadow-lg shadow-black/20" />
          
          <div className="mt-10 bg-black/20 backdrop-blur-sm rounded-2xl p-6 text-left border border-white/10">
            <div className="flex justify-between text-sm font-bold mb-3">
              <span>Tier Progress</span>
              <span>{points >= maxTierPoints ? 'Max Tier Reached' : `${maxTierPoints - points} pts to next tier`}</span>
            </div>
          ...
          <div className="mt-6 flex items-center justify-center gap-2 text-sm font-medium text-white/70">
            <Clock size={16} /> Your points expire in 45 days — earn to reset the clock.
          </div>
        </div>
      </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="bg-white rounded-2xl p-5 border border-[#e8dfd5] shadow-sm">
            ...
            <div className="h-2 bg-[#f0e9e1] rounded-full overflow-hidden">
              <div className="h-full bg-[#c25e30] w-1/3" />
            </div>
          </div>
        </div>
      </div>
