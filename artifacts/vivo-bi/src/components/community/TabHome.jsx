        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <span className="font-bold text-[#2c2a29]">@{post.author.username}</span>
      ...
      <div className="mt-4">
        ...
        {post.taggedProducts?.length > 0 && (
          <div className="flex flex-wrap gap-2 mb-4">
            {post.taggedProducts.map(prod => (
              <button key={prod.id} className="bg-[#f5ece4] text-[#C43E00] text-xs font-semibold px-3 py-1.5 rounded-full hover:bg-[#eaddd1] transition-colors">
                {prod.name}
              </button>
            ))}
          </div>
        ...
        <div className="flex items-center gap-6 border-t border-[#f0e9e1] pt-4 mt-2">
          <PointsAction onClick={toggleLike} points={5}>
            <button data-testid="like-btn" className={`flex items-center gap-1.5 font-medium transition-colors ${liked ? "text-[#C43E00]" : "text-[#7a746e] hover:text-[#C43E00]"}`}>
              {liked ? <Heart size={20} weight="fill" /> : <Heart size={20} weight="bold" />}
              <span>{likesCount}</span>
            </button>
          </PointsAction>
          <button className="flex items-center gap-1.5 font-medium text-[#7a746e] hover:text-[#C43E00] transition-colors">
            <ChatCircle size={20} weight="bold" />
            <span>{post.comments}</span>
          </button>
          <button className="flex items-center gap-1.5 font-medium text-[#7a746e] hover:text-[#C43E00] transition-colors ml-auto">
      ...
      <div className="lg:col-span-4 space-y-6">
        ...
        {featuredChallenge && (
          <div className="bg-gradient-to-br from-[#ebdcd0] to-[#e1ccb9] rounded-2xl p-6 shadow-sm border border-[#d2ba9f]">
            <div className="inline-block px-2.5 py-1 bg-[#C43E00] text-white text-xs font-bold uppercase tracking-wider rounded mb-4">
              Featured Challenge
            </div>
            ...
            <div className="flex items-center justify-between gap-3 mb-5">
              <span className="text-sm font-semibold text-[#C43E00]">⏳ {featuredChallenge.deadline}</span>
              <span className="text-[11px] font-bold text-[#7a746e] text-right">Earn {featuredChallenge.points} pts when published</span>
            </div>
            <button 
            ...
            >
              Enter the Challenge <ArrowRight size={16} weight="bold" />
            </button>
          </div>
        )}
        ...
        <div className="bg-white rounded-2xl p-6 shadow-[0_2px_10px_rgba(44,42,41,0.04)]">
          <h3 className="text-sm font-bold uppercase tracking-widest text-[#a8a199] mb-4">This Week's Jewel</h3>
          ...
          <blockquote className="italic text-[#7a746e] text-sm border-l-2 border-[#FE5000] pl-3">
