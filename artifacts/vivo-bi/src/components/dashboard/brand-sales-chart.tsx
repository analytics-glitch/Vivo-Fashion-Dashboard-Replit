import { useGetSalesByBrand } from "@workspace/api-client-react";
...
import { CHART_COLORS, CHART_COLOR_LIST } from "@/lib/utils";

export function BrandSalesChart({ isDark }: { isDark: boolean }) {
  const { data, isLoading, isFetching } = useGetSalesByBrand();
  ...
  return (
    <Card className="shadow-sm border-muted/60">
      <CardHeader className="px-5 pt-5 pb-2 flex-row items-center justify-between space-y-0">
        <CardTitle className="text-base font-serif">Portfolio Brand Mix</CardTitle>
        {!loading && sortedData.length > 0 && (
          <CSVLink 
            data={sortedData} 
            filename="brand-sales.csv" 
      ...
      <CardContent className="p-5 pt-0">
        {loading ? (
          ...
          <ResponsiveContainer width="100%" height={320} debounce={0}>
            <BarChart data={sortedData} layout="vertical" margin={{ top: 20, right: 30, left: 20, bottom: 0 }}>
              ...
              <YAxis dataKey="brand" type="category" width={80} tick={{ fontSize: 12, fill: tickColor, fontWeight: 500 }} stroke={tickColor} axisLine={false} tickLine={false} />
              <Tooltip content={<CustomTooltip />} isAnimationActive={false} cursor={false} />
              <Bar dataKey="revenue" name="Revenue" fillOpacity={0.9} activeBar={{ fillOpacity: 1 }} isAnimationActive={false} radius={[0, 4, 4, 0]} barSize={28}>
