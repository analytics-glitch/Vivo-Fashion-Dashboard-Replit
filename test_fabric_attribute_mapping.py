...
class _StateCursor:
    ...
    def __init__(self, stored_signature=None):
    ...
    def execute(self, sql, params=None):
        ...
        if norm.startswith("select value from fabric_sync_state"):
            self._last = ((self.stored_signature,)
                          if self.stored_signature is not None else None)
        else:
            self._last = None
...
def _product_105435_record():
    """Shaped like the reported live product: width attribute populated,
    attribute GSM empty (False), text GSM carrying the value."""
    return {
        "id": 682432,
        "name": "AL SAWAE Tiktok Satin - Beige/Brown Print",
        "default_code": False,
...
class ReconcileWiringTests(unittest.TestCase):
    ...
    def setUp(self):
        self.meta = _required_meta(x_gsm_text={"string": "GSM", "type": "char"})
    ...
    def test_mapping_change_promotes_incremental_to_full_reconcile(self):
        ...
        models = self._run(cur, [_product_105435_record()], since=self.SINCE)
        ...
        self.assertTrue(any(s.startswith("truncate raw_fabric_products")
                            for s in cur.sqls()))
        # Both GSM candidates were requested from Odoo.
        self.assertIn("x_gsm_attr", models.search_fields[0])
        self.assertIn("x_gsm_text", models.search_fields[0])
        # The row carries the repaired values: width from the attribute, GSM
        # from the text fallback, and the standard formula derives 0.2669.
        (_, rows), = self.captured
        row = rows[0]
        self.assertEqual(row[0], 682432)
    ...
    def test_ambiguous_primary_aborts_before_any_table_write(self):
        ...
        cur = _StateCursor()
        with self.assertRaisesRegex(RuntimeError, r"gsm.*ambiguous"):
            self._run(cur, [_product_105435_record()], since=None)
        self.assertFalse(any(s.startswith("truncate") for s in cur.sqls()))
