...
- `/loyalty-app` — standalone Shopify-connected loyalty PWA vendored AS-IS from an external repo into repo-root `loyalty-app/` (npm, NOT part of the pnpm workspace; own design, Fastify + Prisma + cookie/JWT auth — its `loyalty-app/replit.md` rules are non-negotiable). Runs as its own process (port 8090) via a second `[[services]]` block on the api-server artifact; SPA + `/loyalty-app/api/*` + `/loyalty-app/webhooks/shopify/*` are all same-origin under the base path. Prisma tables live in the dedicated `loyalty_app` Postgres schema (NEVER `public` — a `db push` there would drop BI tables; always `migrate deploy` via `loyalty-app-backend/scripts/db-env.mjs`). Missing secrets degrade gracefully (ephemeral JWT warning, Google/SMTP/Shopify features disabled until `LOYALTY_APP_*` secrets are set). Separate from the existing `/loyalty/` member app and `clerk_auth_gate` (which ignores `/loyalty-app`). See `.agents/memory/loyalty-app-vendored-pwa.md`.
