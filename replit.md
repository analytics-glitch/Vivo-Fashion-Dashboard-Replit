...
- SOPs (`/sops`): 5 workflow stages × 11 fixed department folders, including Awaiting Approval. Files are BYTEA with an audited rich-text revision in `sop_files` (20 MB upload cap). Existing upload grants govern submissions; admins + Stephen + Frankie review/edit/retire, while admins + Stephen approve. Transitions and optimistic edit revisions are server-enforced.
