...
- SOPs (`/sops`): 4 workflow stages × 11 fixed department folders; files are BYTEA in `sop_files` (20 MB cap). Existing upload grants govern submissions; admins + Stephen + Frankie review/retire, admins + Stephen approve; transitions are server-enforced and audited.
