...
**Trap: `raw_shopify_orders.total_price` is 0.0 on every row** (the historical extract never populated it). Order value must come from joining line-level `shopify_sales` (`order_id::text = o.id AND store_id`, `sale_kind='order'`) — join coverage verified 100%. `shopify_sales.total_sales` is STORE-LOCAL currency: divide UGX/RWF by `currency_rates`.

**Why:** the first endpoint version read header total_price and showed orders with 0 KES for the whole Shopify era.

**How to apply:** any new intraday/hourly surface must reuse this source split and the shopify_sales value join; `sync_incremental.py` upserts both header tables each cycle (no extra API calls) so the data stays fresh in dev AND prod. Online (`raw_shopify_vendor_sales`) is daily-grain — exclude it and say so in the UI.
