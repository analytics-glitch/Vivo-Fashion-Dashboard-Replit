...
**How to apply:** Any New/Returning/Walk-in customer breakdown should trust `customer_type`. Counting `COUNT(DISTINCT order_id)` (orders) is the reliable volume — distinct `customer_id` is unreliable for POS. Note `/api/customers` (the Customers-page headline KPIs) still uses the first_purchase approach and therefore over-reports "New" for POS-heavy windows; if asked to fix those headline figures, apply the same customer_type-based logic.
