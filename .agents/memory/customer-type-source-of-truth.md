...
## Exec Summary uses a DIFFERENT identified-universe than get_customers
`_es_customer_windows` (Exec Summary KPIs) and `get_customers` (Customers page) both classify New=first-ever-in-window / Returning=first-before-window, and BOTH now reconcile internally (New+Returning=Total). **But their universes differ:** `_es_customer_windows` gates on `customer_id IS NOT NULL/NOT IN ('None','null','')` + `BASE_FILTERS` and computes first_ever on the *raw* `customer_id`; `get_customers` gates on `customer_type IN ('new','returning','registered')` and uses the **unified first-purchase bridge** (`_unified_first_purchase_ctes`) across the 2026-03-20 Kenya id switch. So absolute New/Returning/Total can still diverge between the two pages (the id-bridge especially can mislabel post-cutover Kenya as New). Audit F08 fixed only the within-Exec-Summary reconciliation (was: `oc=1` filter dropped multi-order customers from both buckets). Aligning the two universes (bridge + customer_type gating) is the remaining parity work (F04 territory).
