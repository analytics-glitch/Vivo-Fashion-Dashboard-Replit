...
Product styles link to one canonical Level 3 base fabric from Fabric BI's authoritative `fabric_name`. Derive it from the Level 4 product name only when `fabric_name` is blank. Weekly colour allocations select Level 4 records by barcode; names are display labels, not identity.
