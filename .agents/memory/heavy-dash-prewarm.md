...
- The PG buffer-warm scan (90d all_sales + inventory/products counts) runs only the FIRST 3 cycles after boot — buffers stay hot from the warm targets themselves; as a forever-loop it was itself a perpetual ~16s slow-query every 600s.
- The warmer also self-heals rollups: each cycle it checks rollup_meta (count of current-schema_ver rows + max age); if any core rollup is missing or >90 min old it Popen-kicks `build_sales_rollups.py` (single-flighted + watermark-skipped internally), throttled to one kick per 20 min. This is the prod bootstrap path for a fresh DB — do not remove it when touching the warm loop.
...
- Diagnostic trap: `sync_health_log` slow_query rows recurring at ~600s intervals are the PREWARMER re-computing warm views (background load), not user-facing latency. Before rewriting such a query into a rollup, run it standalone with EXPLAIN — the "13s" range-mgmt/IBT style×store CTEs measured 2.7s alone; the log times were inflated by concurrent prewarm cycles.
