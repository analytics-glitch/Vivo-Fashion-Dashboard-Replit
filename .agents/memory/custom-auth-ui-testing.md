...
**Cold cache after an api-server restart:** dashboard tabs Promise.all several endpoints behind one loading overlay, so first uncached loads can read as "stuck" — have testers wait ~60s per filter change before calling failure.
