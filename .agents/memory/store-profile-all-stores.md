...
- Inventory predicate becomes `_SP_ALL_INV_PRED`, a positive join to active `pos_locations` rows with `store_type='store'`; never use an exclusion list because coded WIP names such as FINPR/Stock can slip through.
...
- The All Stores Inventory KPI is the literal `SUM(all_inventory.available)` across those active retail stores. Product classification may explain less stock but must never reduce the headline total or lifecycle denominator.
- Stock Diagnosis's Primary colour breakdown reads Odoo's dedicated finished-product Primary Color field, not name-derived `color_print`; missing values are Unspecified and print names must never appear.
