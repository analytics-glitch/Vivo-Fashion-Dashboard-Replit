...
**Gitignored build trap (deploy):** `dashboard/.gitignore` ignores the whole `/build` dir, so `dashboard/build/fabric.html` is UNTRACKED and never ships on publish; the deploy build step doesn't rebuild CRA either. Fix: the `/fabric` handler serves repo-root **`fabric_dashboard_live.html` (TRACKED canonical) first**, falling back to `build/fabric.html`. **How to apply:** edit `dashboard/build/fabric.html` while iterating, then `cp dashboard/build/fabric.html fabric_dashboard_live.html` so the two stay byte-identical — the tracked copy is what production actually serves. If they drift, prod shows stale UI.
