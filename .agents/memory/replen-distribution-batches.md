...
**How to apply:** the WRITE distribution endpoints (`POST /api/replenishment/distribute`, `DELETE .../distributions/{id}`) are roster-manager-gated (`_can_manage_roster`). `GET .../distributions` is intentionally open to ANY authenticated user — the frozen batch IS the pickers' work order and they mark their own lines done, so gating GET to managers hid the batch from pickers (they fell back to the live 400-row SOR list = "seeing a different thing / the 400"). On the web page the LIVE SOR pick list card (and its heavy `loadSor` compute + the mark-done refresh of it) is gated behind `isAdmin` so pickers see ONLY the frozen batches; the BatchCard delete button is gated behind a `canDelete={isAdmin}` prop. The frontend `openKeys` must be a `useMemo` dependency of `visibleRows` or the live list goes stale when batches change without a `rows` identity change.
