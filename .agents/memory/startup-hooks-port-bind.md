...
**How to apply:** New startup ensures → `@_deferred_startup` (order = registration order; keep dependents after their dependencies, e.g. seed-admin after users-table). Keep synchronous only fast non-locking asserts (e.g. standard_conforming_strings). The watchdog also waits (bounded 60s port poll) for the API port before starting the sync loop, so sync writes never race boot. Runner lifecycle lines log at WARNING because the `api_pg` logger has no handler — INFO is silently dropped; deployment-log observability depends on WARNING+.
