...
- **Spotlights:** a Community Spotlight is an `article_type="spotlight"` row in that same model, with subject, byline and gallery metadata; each rotating jewel carries its article slug.
  **Why:** Stories need the existing member-only comment, moderation, DPA and URL behavior without splitting audience discussion across a second article system.
  **How to apply:** Seed launch spotlight rows only when missing (never overwrite a later interview or gallery update), and make every Spotlight CTA open `?page=article-{slug}` rather than the generic Community tab.
