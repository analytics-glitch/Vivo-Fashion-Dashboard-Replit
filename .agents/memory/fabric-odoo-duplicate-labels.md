...
**Rule:** Odoo `fields_get` display labels are NOT unique and never will be — the live catalogue legitimately carries an attribute-backed many2one AND a plain char field under the identical label (e.g. "GSM"). The Fabric metadata resolver must map each business concept to an ORDERED candidate list ranked first by business-source alias (direct values before “From Fabric Reference” fallbacks), then by field type (relational/selection → numeric → char/text), and take the first POPULATED value per product. Only a true tie (two same-type fields under the same alias at the head of a required concept) is genuinely ambiguous — that still fails safely BEFORE any TRUNCATE so the last known-good master survives.
...
**Related-field refresh:** edits to Fabric Reference attributes can update a variant’s related Width/GSM values without advancing `product.product.write_date`. The fast pull must re-read incomplete Fabric master rows alongside its normal write-date window. Keep this pass bounded but rotate its persisted cursor in the same transaction as the product update, so a large incomplete population is eventually covered and a failed pull cannot skip a row.
...
**Why:** a duplicate "GSM" label (attribute + text twin) hard-stopped every Fabric product sync for ~40 min in Aug 2026; treating valid duplicates as fatal ambiguity means any Odoo admin adding a similarly-labelled helper field can silence the whole feed. Related values also do not reliably touch the variant timestamp, so a conventional incremental window can preserve false “missing Width/GSM” exclusions indefinitely.

**How to apply:** when adding a new Fabric business attribute, add label aliases to the spec — never assume a unique label or a fixed `x_vivo_attr_NN` id. Put direct aliases before reference aliases; use type preference only within an alias. If source selection logic changes, rely on the signature promotion (don't hand-run backfills). Data Quality's missing-kg-per-metre view only lists incomplete products that have real stock or usage — zero-quantity incomplete products are by design absent there but still carry `kg_per_mtr_src='incomplete'` everywhere else.
