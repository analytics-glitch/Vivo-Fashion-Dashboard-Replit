...
- **Own brands only (added Aug 2026):** the catalogue universe excludes `brand ILIKE '%third party%'` on ALL THREE endpoints — search, facets AND style-card (per-SKU-row, before style grouping, so mixed-brand styles keep own-brand sizes; style-card 404s when nothing remains). Keep the three in lockstep, and bump the facets cache key whenever the universe definition changes.
